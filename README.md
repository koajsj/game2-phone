...
- 对头部、肩部和髋部关键点加入可见度阈值，遮挡或识别质量不足时暂停评分
- 连续多帧未识别到完整上半身后清空评分，避免用户误读上一次结果
- 继续保留趋势曲线、会话统计和中文纠正建议的本地分析模式
...
- 尽量让头部、肩部和髋部都进入画面。
- 如果评分突然变成待机，请先调整光线、距离和身体位置，让关键点重新完整可见。
- 手机端建议竖屏打开，并允许摄像头权限。
